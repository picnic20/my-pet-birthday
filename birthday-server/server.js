...
// 💡 1. Firebase Admin SDK 최신 ESM 표준 문법으로 가져오기
import { initializeApp, cert } from 'firebase-admin/app';
import { getAuth } from 'firebase-admin/auth';

// 💡 2. 우리 프로젝트 안의 파일들 가져오기
...
// 1. Express에게 EJS 템플릿 엔진을 사용하겠다고 선언 (npm install ejs 필요)
...
// 1. Render 환경변수에 등록해 둔 포트원 API 시크릿 키로 클라이언트 초기화
const portoneClient = new PortOneClient({
  secret: process.env.PORTONE_API_SECRET 
});
/* 🏠 사용자가 메인 페이지(/)에 접속했을 때 화면을 그려주는 Node.js 라우터
 */
app.get('/', (req, res) => {
  // 내 .env 파일에 적어둔 공개키값들을 객체로 묶습니다.
  const firebaseKeys = {
  ...
  // views/index.ejs 파일을 그리면서 위의 키값들을 데이터로 넘겨줍니다.
  res.render('index', { firebaseKeys });
});

// 미들웨어 설정
app.use(cors());
app.use(express.json()); // 프론트엔드가 보낸 JSON 데이터를 읽기 위해 필수!
...
// Render 환경변수(FIREBASE_CONFIG)에 저장된 비밀키 JSON 스트링을 오브젝트로 파악합니다.
// =================================================================
const firebaseConfigRaw = process.env.FIREBASE_SERVICE_ACCOUNT;
if (!firebaseConfigRaw) {
  console.error("❌ Render 환경변수에 FIREBASE_SERVICE_ACCOUNT가 설정되지 않았습니다!");
  process.exit(1);
}
...
try {
 ...
 initializeApp({
  ...
  console.error("❌ Firebase 초기화 중 JSON 파싱 에러 발생:", error);
  process.exit(1);
}
...
// =================================================================
// 3. [API 엔드포인트] 회원가입 및 로그인 처리
// 프론트엔드의 FirebaseUI에서 인증 성공 후 이 주소로 토큰을 쏩니다.
// =================================================================
app.post('/api/login', authMiddleware, async (req, res) => {
  try {
    // authMiddleware(문지기)가 토큰을 검사한 후, 
    // 이미 MongoDB에서 유저를 찾거나 새로 생성해서 req.user에 넣어두었습니다.
    const user = req.user;
        if (!user) {
    ...
    // 프론트엔드에게 성공 메시지와 함께 현재 유저의 크레딧 정보를 내려줍니다.
    return res.status(200).json({
      ...
      data: {
        ...
        credits: user.credits // 프론트엔드에서 상단바에 "보유 크레딧: 3개" 처럼 띄울 때 사용
      }
    });
 } catch (error) {
...
// =================================================================
// 4. [보안 예시 API] 크레딧 잔액 조회 (로그인한 사람만 가능)
// =================================================================
app.get('/api/user/profile', authMiddleware, (req, res) => {
  // authMiddleware 덕분에 로그인 안 한 사람은 이 코드까지 들어오지도 못하고 튕겨나갑니다.
  return res.status(200).json({
    success: true,
...
/**
 * 💳 [API] 포트원 결제 완료 최종 검증 및 크레딧 충전 엔드포인트
 * 프론트엔드에서 결제창이 닫힌 직후 이 API를 호출하게 됩니다.
 */
app.post('/api/payments/complete', authMiddleware, async (req, res) => {
  // authMiddleware 덕분에 req.user에 현재 로그인한 MongoDB 유저 정보가 안전하게 들어있습니다.
  const currentLoggedInUserId = req.user._id; 
  const { paymentId, amount } = req.body; // 프론트엔드가 보내준 영수증 ID와 결제 금액

  try {
    // [Step 1] 포트원 서버에 직접 물어봐서 실제 결제 내역(영수증)을 가져옵니다.
    const paymentData = await portoneClient.payment.getPayment({ paymentId });

    // [Step 2] 삼각 대조 검증 
    // 1) 포트원 시스템상 진짜로 결제가 완료(PAID)되었는지 확인
    // 2) 유저가 실제로 낸 돈이 우리가 프론트에서 요청한 금액(amount)과 정확히 일치하는지 확인
    if (paymentData.status === "PAID" && paymentData.amount.total === amount) {
      
      /* // [선택 사항] 만약 Payment(결제이력) 내역을 DB에 따로 기록하신다면 아래 검증을 추가합니다.
      const orderRecord = await Payment.findOne({ orderId: paymentData.paymentId });
      if (!orderRecord) {
        return res.status(404).json({ message: "존재하지 않는 주문 번호입니다." });
      }
      // 가로채기 방지: 영수증 주인과 결제 요청 유저 ID 대조
      if (orderRecord.userId.toString() !== currentLoggedInUserId.toString()) {
        return res.status(403).json({ message: "결제 요청자 정보가 일치하지 않습니다." });
      }
      orderRecord.status = 'PAID';
      await orderRecord.save();
      */

      // [Step 3] 금액별 충전할 크레딧 계산 (예: 10,000원이면 10크레딧 / 1,000원당 1크레딧)
      const creditsToCharge = amount / 1000; 

      // [Step 4] ★ 핵심: 안전한 $inc 연산자로 유저 잔여 크레딧 누적 업데이트
      const updatedUser = await User.findByIdAndUpdate(
        currentLoggedInUserId,
        { $inc: { credits: creditsToCharge } },
        { new: true } // 업데이트가 완료된 후의 최신 유저 정보를 리턴받음
      );
      ...
      // 결제 상태가 PAID가 아니거나 금액이 다르면 위변조 시도로 간주하고 거절
  ...
  app.use('/*splat', async (req, res, next) => {
    ...
    try {
      ...
      if (result && result.code) {
        res.status(200).set({ 'Content-Type': 'application/javascript' }).end(result.code);
      } else {
        next();
      }
    } catch (e) {
